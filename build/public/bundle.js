...
function generarContrasena(longitud, incluirEspeciales, incluirNumeros, incluirMayusculas ) {
  ...
  var letrasMayusculas = "ABCDEFGHIJKLMOPQRSTUVWXYZ"
...
function cajaFuerte(codigoSecreto, cantidadIntentos){
  ...
  for (let i = 0; i < codigoSecreto.length; i++) {
    if(Number(codigoSecreto[i]) || Number(codigoSecreto[i]) === 0) continue
    else return "El codigo secreto solo puede estar conformado por numeros"
  }
...
function eliminarActividad(indice){
    ...
    if(indice < 0 || indice >= actividadesSospechozas.length){
        ...
        // con un bucle for esta parte 
        // var nuevasActividades = []
        // for (var i = 0; i < actividadesSospechozas.length; i++) {
        //     if (i === indice) {
        //         continue
        //     }else nuevasActividades.push(actividadesSospechozas[i])
        // }
        // actividadesSospechozas = nuevasActividades

        // con filter
        //actividadesSospechozas = actividadesSospechozas.filter((actividad, index) => index !== indice)
...
function filtrarActividadesPorRiesgo(nivelRiesgo){
    ...
    if(nivelRiesgo !== "bajo" && nivelRiesgo !== "medio" && nivelRiesgo !== "alto"){
        ...
        var filtrado = actividadesSospechozas.filter(actividad => actividad.includes(nivelRiesgo))
        if(filtrado.length === 0) return "No hay actividades con este nivel de riesgo"
...
function generarReporteDeActividades(){
    /* TU CODIGO */
    const actividades = actividadesSospechozas.map((actividad, indice) => "Id: " + indice + " " + actividad)
    if(actividades.length === 0) return "No hay actividades para mostrar"
...
var asistente = {
    verPerfiles: function(opcion){
        ...
        else if(opcion === "nombre") return perfiles.map(perfil => perfil.usuario)
        else if(opcion === "codigo") return perfiles.map(perfil => perfil.codigo)
        else if(opcion === "nivel") return perfiles.map(perfil => perfil.nivel_de_autorizacion)
        else if(opcion === "antiguedad") return perfiles.map(perfil => perfil.antiguedad)
    },
    
    verPerfilesPorAntiguedad: function(){
        /* TU CODIGO */
        var perfilesPorAntiguedad = []
        for(let i = 0; i < perfiles.length; i++){
                perfilesPorAntiguedad.push(perfiles[i])
        }
        return perfilesPorAntiguedad.sort((a, b) => b.antiguedad - a.antiguedad)
    },
    ...
    modificarAcceso: function(usuario, codigo){
        /* TU CODIGO */
        if(!perfiles.find(perfil => perfil.usuario === usuario)) return "usuario no encontrado"
        else if(codigo.length !== 4 || isNaN(Number(codigo))) return "codigo de acceso invalido, debe contener solo 4 numeros"
        else {
